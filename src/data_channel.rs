            ...
            match TcpListener::bind(&data_addr) {
                Ok(listener) => {
                    if listener.set_nonblocking(true).is_ok() {
                        ...
                        let response = format!("227 Entering Passive Mode (127.0.0.1:{})\r\n", port);
                        let _ = cmd_stream.write_all(response.as_bytes());
                        info!("Data channel ready on {} for client {}", data_addr, client_addr);
                    } else {
...
pub fn setup_data_stream(
    ...
    let listener = {
        let mut clients_guard = clients.lock().unwrap();
        clients_guard.get_mut(client_addr).and_then(|client| client.take_data_listener())
    };
    ...
    if let Some(listener) = listener {
        for _ in 0..ACCEPT_ATTEMPTS {
            match listener.accept() {
                Ok((data_stream, addr)) => {
                    info!("Data connection accepted from {} for client {}", addr, client_addr);
        ...
        error!("Timeout ({} seconds) waiting for data connection from {}", ACCEPT_TIMEOUT_SECS, client_addr);
        let mut clients_guard = clients.lock().unwrap();
        if let Some(client) = clients_guard.get_mut(client_addr) {
            client.set_data_listener(Some(listener));
        }
